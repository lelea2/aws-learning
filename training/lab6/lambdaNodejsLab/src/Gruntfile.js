...
module.exports = function(grunt) {
        ...
        options: {
          ...
          then: function(answers, done) {
            ...
            s3bucketinp.createBucket(function(err, data) {
              if (err) {
                console.log(answers.inputBucket + ' could not be created, ensure bucket does not already exist and bucket name does not contain upper case characters')
              }
              else {
            ...
            s3bucketout.createBucket(function(err, data) {
              if (err) {
                console.log(answers.outputBucket + ' could not be created, ensure bucket does not already exist and bucket name does not contain upper case characters')
              }
              else {
  ...
  grunt.registerTask('test', function() {
    ...
    var outputKey = event.Records[0].s3.object.key;
